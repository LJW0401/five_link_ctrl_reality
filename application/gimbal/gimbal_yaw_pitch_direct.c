...
void GimbalReference(void) 
{
  ...
  {
    if (gimbal_direct.last_mode != GIMBAL_TEST)
    {
      gimbal_direct.reference.pitch = gimbal_direct.feedback_pos.pitch;
    }
    else
...
void GimbalSendCmd(void) 
{
    ...
    ModifyDebugDataPackage(0, gimbal_direct.reference.pitch,"set");
    ModifyDebugDataPackage(1, gimbal_direct.pitch.fdb.pos,"fdb");
    ModifyDebugDataPackage(2, gimbal_direct.pitch.set.curr, "curr");
}
