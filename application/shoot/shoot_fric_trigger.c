...
inline int ShootGetFricState(void) { return fric_ui; }

inline int ShootGetTriggerState(void) { return trigger_ui; }
...
void ShootSetMode(void)
{
    ...
    else if (switch_is_mid(SHOOT.rc->rc.s[SHOOT_MODE_CHANNEL])) 
    {
        ...
        if ((SHOOT.rc->mouse.press_l && !SHOOT.shoot_flag) || (SHOOT.rc->key.v & KEY_PRESSED_OFFSET_G) && (!SHOOT.shoot_flag)) 
        {
            SHOOT.mode = LAOD_BULLET;
        } 
        ...
        SHOOT.shoot_flag = SHOOT.rc->mouse.press_l;
        SHOOT.shoot_flag = SHOOT.rc->key.v & KEY_PRESSED_OFFSET_G;
        ...
        if (SHOOT.rc->mouse.press_l || SHOOT.rc->key.v & KEY_PRESSED_OFFSET_G) 
        {
...
void ShootSendCmd(void)
{
    ...
    else if (TRIGGER_MOTOR_TYPE == DM_4310) 
    {
        ...
        // ModifyDebugDataPackage(1, SHOOT.REF.trigger_speed_ref, "trigger_set");
